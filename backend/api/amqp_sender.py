...
class SenderAMQP:
    ...
    def send_task(self, task_id, fname, furl, arch="", bits="", db_content=None, db_name=None):
        """
        ...
        msg = self.encode(task_id, fname, furl, arch, bits, db_content, db_name)
        self.send_to_queue(msg)
            
    def encode(self, task_id, fname, furl, arch, bits, db_content, db_name):
        msg = SECRET_KEY + b'\0'.join((str(task_id).encode(), fname.encode(), furl.encode(), arch.encode(), str(bits).encode())) + b'\0'
        if db_content != None:
